...
namespace bafprp
{
	...
	void MYSQL::log( LOG_LEVEL level, const std::string& log )
	{
	...
	void MYSQL::checkDB( property_map& props )
	{
		LOG_TRACE( "MYSQL::checkDB" );
		...
		if( !_dbConnected )
		{
			printf( "Failed to connect to the database: %s, mysql output will not be available\n", _database.c_str() );
		}
	...
	std::string MYSQL::sanitize( const std::string& string )
	{
