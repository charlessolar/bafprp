...
namespace bafprp
{
	...
	void Console::error( const BafRecord* record, const std::string& error )
	{
	...
	void Console::record( const BafRecord* record )
	{
		...
		while( ( field = record->getNextField( lastUID ) ) != NULL )
		{
			...
			if( !field->filter() ) std::cout << field->getDesc() << ": " << field->getString() << std::endl;
		}
